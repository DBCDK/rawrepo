...
docker_image=docker-io.dbc.dk/rawrepo-maintain-1.13-snapshot
version=${USER}
port=`id -u ${USER}`1
detached="-d"
...
if [ "$version" = "${USER}" ]
then
    docker build -t ${docker_image}:${USER} maintain/.
fi
...
container_id=`docker run -it ${detached} -p ${port}:8080 \
		-e RAWREPO_DB_URL="rawrepo:thePassword@localhost:${RAWREPO_PORT}:5432/rawrepo" \
