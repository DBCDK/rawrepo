...
public class NotifyDeployScriptTest {
    private final static MOUNT_POINT_NAME = 'unittest'
    private final static ARTIFACT_NAME = "rawrepo-notify.war"
    private final static ARTIFACT = NotifyDeployScriptTest.getResource( "/" + ARTIFACT_NAME )
    private final static CONTEXT = "/rawreponotify"
    ...
    class GluScript extends NotifyDeployScript {
    ...
    void configure_addsLoggingProperties() {
        def instance = new GluScript( shellImpl, [ artifact: ARTIFACT, logging: LOGGING, dbUrl: DB_URL, dbUser: DB_USER, dbPassword: DB_PASSWORD, dbQueueWorker: DB_WORKER, pollInterval: POLL_INTERVAL, threadPoolSize: THREAD_POOL_SIZE ] )
        instance.install()
        instance.deployer = new GlassFishAppDeployer(['baseUrl':'url']) {
            Object deploy(Map<String, String> appProperties) {
                assertThat(appProperties.name, is(instance.appName))
    ...
    void configure_whenInstalled_exposesLogFiles() {
        ...
        def expected = [
            "rawreponotify.logstash",
            "rawreponotify.plain",
            "rawreponotify-error.plain",
        ]
    ...
    void configure_whenInstalled_configuresEjbXml() {
        ...
        assertThat( ejbXml.evaluateXpath( "/ns:ejb-jar/ns:enterprise-beans/ns:session[1]/ns:env-entry[1]/ns:env-entry-name", STRING ) as String, is( "workerName" ) )
        assertThat( ejbXml.evaluateXpath( "/ns:ejb-jar/ns:enterprise-beans/ns:session[1]/ns:env-entry[1]/ns:env-entry-value", STRING ) as String, is( DB_WORKER ) )
